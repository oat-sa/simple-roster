...
```shell script
docker container exec -it simple-roster-phpfpm bash -c "source .env.test && vendor/bin/phpunit"
```

To run the full test suite with all the necessary coverage reports (for Infection and coverage checker):

```shell script
docker container exec -it simple-roster-phpfpm bash -c "source .env.test && XDEBUG_MODE=coverage vendor/bin/phpunit --coverage-xml=var/log/phpunit/coverage/coverage-xml --coverage-clover=var/log/phpunit/coverage.xml --log-junit=var/log/phpunit/coverage/junit.xml"
```
