<?php

declare(strict_types=1);

namespace OAT\SimpleRoster\Service\Rostering\Dto;

final class RosteringUserRow
{
    public const FIELD_USER_USERNAME = 'user_username';
    public const FIELD_USER_PASSWORD = 'user_password';
    public const FIELD_USER_ORGANIZATION_ID = 'user_organizationId';
    public const FIELD_SESSION_NAME = 'session_name';
    public const FIELD_USER_LANGUAGE = 'user_language';
    public const FIELD_USER_ACTIVE = 'user_active';

    private function __construct(
        private readonly ?string $userUsername,
        private readonly ?string $userPassword,
        private readonly ?string $userOrganizationId,
        private readonly ?string $sessionName,
        private readonly ?string $userLanguage,
        private readonly ?string $userActive
    ) {
    }

    /**
     * @param array<string, string> $values
     */
    public static function fromNormalizedRow(array $values): self
    {
        return new self(
            self::fieldValueOrNull($values, self::FIELD_USER_USERNAME),
            self::fieldValueOrNull($values, self::FIELD_USER_PASSWORD),
            self::fieldValueOrNull($values, self::FIELD_USER_ORGANIZATION_ID),
            self::fieldValueOrNull($values, self::FIELD_SESSION_NAME),
            self::fieldValueOrNull($values, self::FIELD_USER_LANGUAGE),
            self::fieldValueOrNull($values, self::FIELD_USER_ACTIVE)
        );
    }

    public function isImportable(): bool
    {
        return $this->userUsername !== null
            || $this->userPassword !== null
            || $this->userOrganizationId !== null
            || $this->sessionName !== null
            || $this->userLanguage !== null
            || $this->userActive !== null;
    }

    public function getUserUsername(): ?string
    {
        return $this->userUsername;
    }

    public function getUserPassword(): ?string
    {
        return $this->userPassword;
    }

    public function getUserOrganizationId(): ?string
    {
        return $this->userOrganizationId;
    }

    public function getSessionName(): ?string
    {
        return $this->sessionName;
    }

    public function getUserLanguage(): ?string
    {
        return $this->userLanguage;
    }

    public function getUserActive(): ?string
    {
        return $this->userActive;
    }

    /**
     * @param array<string, string> $values
     */
    private static function fieldValueOrNull(array $values, string $field): ?string
    {
        if (!array_key_exists($field, $values)) {
            return null;
        }

        return $values[$field] === '' ? null : $values[$field];
    }
}
